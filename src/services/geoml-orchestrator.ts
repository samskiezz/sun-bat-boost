...
export async function runPolygonIntercoordination(params: {
  ...
  recordMsg({ from: "Embedder", to: "VectorIndex", topic: "UPSERT", content: { signature: embedRes.signature } });
  ...
  recordMsg({ from: "CatalogMatcher", to: "DesignValidator", topic: "CANDIDATES", content: matchRes.matches, confidence: 0.9 });
  ...
  if (postcode) {
    ...
    if (t) {
      ...
      recordMsg({ from: "TariffRecommender", to: "ROIEngine", topic: "PLAN", content: tariff });
    }
  }
  ...
  recordMsg({ from: "ROIEngine", to: "SystemManager", topic: "SUMMARY", content: { tariff, rebates, roi }, confidence: 0.95 });
