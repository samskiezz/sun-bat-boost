...
export async function getDnspByPostcode(postcode: string | number): Promise<DnspDetails> {
  ...
  try {
    ...
    // Final fallback: use state-based DNSP mapping - THIS SHOULD ALWAYS WORK
    console.log('Using state-based fallback DNSP mapping for postcode:', postcodeNum);
    ...
    dnspCache.set(cacheKey, dnspDetails);
    console.log(`✅ Fallback DNSP mapping for postcode ${postcodeNum} in ${state}: ${defaultDnsp.network}`);
...
export function getStateFromPostcode(postcode: string | number): string {
  ...
  // Get state from postcode (fallback if DNSP lookup fails)
  if (postcodeNum >= 5000 && postcodeNum <= 5199) return 'SA'; // Fixed SA metro range
  if (postcodeNum >= 5200 && postcodeNum <= 5999) return 'SA'; // Fixed SA rural range
