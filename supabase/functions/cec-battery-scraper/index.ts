...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Remove duplicates based on brand + model combination
    const uniqueBatteries = batteries.reduce((acc, current) => {
      const key = `${current.brand.trim()}-${current.model.trim()}`;
      if (!acc.has(key)) {
        acc.set(key, current);
      }
      return acc;
    ...
    const deduplicatedBatteries = Array.from(uniqueBatteries.values());
    console.log(`Deduplicated ${batteries.length} batteries down to ${deduplicatedBatteries.length} unique entries`);
    ...
    // Insert new batteries in smaller batches with better error handling
    const batchSize = 50; // Smaller batches to avoid conflicts
    let insertedCount = 0;
    
    for (let i = 0; i < deduplicatedBatteries.length; i += batchSize) {
      const batch = deduplicatedBatteries.slice(i, i + batchSize);
      const batchNumber = Math.floor(i / batchSize) + 1;
      
      try {
        const { error } = await supabase
          .from('batteries')
          .insert(batch); // Use insert instead of upsert since we cleared the table
        ...
        if (error) {
          ...
          const { error: upsertError } = await supabase
            .from('batteries')
            .upsert(batch, { onConflict: 'brand,model', ignoreDuplicates: true });
          
          if (!upsertError) {
            insertedCount += batch.length;
            console.log(`Upserted batch ${batchNumber}/${Math.ceil(deduplicatedBatteries.length / batchSize)}: ${insertedCount} total`);
          }
        } else {
          insertedCount += batch.length;
          console.log(`Inserted batch ${batchNumber}/${Math.ceil(deduplicatedBatteries.length / batchSize)}: ${insertedCount} total`);
        }
      } catch (error) {
